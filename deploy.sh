...
ensure_debug_flag() {
    # Temporarily inject "debug" flag for auto-reload during dev
...
deploy() {
    ...
    ssh $SSH_OPTS "$REMOTE" "rm -rf ${STAGING} && mkdir -p ${STAGING}/dist ${STAGING}/defaults/py_modules"
    ...
    scp $SCP_OPTS \
        ...
        "${REMOTE}:${STAGING}/dist/"
    scp $SCP_OPTS \
        "$SCRIPT_DIR/defaults/py_modules/__init__.py" \
        "$SCRIPT_DIR/defaults/py_modules/network_utils.py" \
        "$SCRIPT_DIR/defaults/py_modules/speed_test_engine.py" \
        "${REMOTE}:${STAGING}/defaults/py_modules/"

    # Swap into plugin directory (sudo handles root-owned target)
    ssh $SSH_OPTS "$REMOTE" "sudo rm -rf '${REMOTE_DIR}' && sudo mv ${STAGING} '${REMOTE_DIR}' && sudo chmod -R 755 '${REMOTE_DIR}'"
